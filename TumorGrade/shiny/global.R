...
data.2ndLR <- filter(a[[8]], 환자번호 %in% a.list$Number) %>% 
...
out$Group2 <- factor(ifelse(out$Group == "Worsening", "Worsening", "Improving + Stable"))
...
out$SecondLR<- factor(as.integer(!is.na(c$Date_2ndLR)))
out$Day_SecondLR <- ifelse(out$SecondLR == 1, as.numeric(c$Date_2ndLR - as.Date(c[["수술날짜\r\n\r\ndd-mm-yyyy.x"]])), out$Day_FU)
